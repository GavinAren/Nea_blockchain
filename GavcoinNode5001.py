...
class Blockchain:
    ...
    def proof_of_work(self, prev_nonce):
        ...
        while check_proof is False: #need to keep incrementing the nonce until roght one is found
            ...
            #**REALISED THAT DONT ACTUALLY NEED A MATHEMATICAL FUNTION TO DO IT55
    ...
    def replace_chain(self): #funtion that spots the longest chain held by any node and then makes sure all other nodes with a shorter chain have the same chain
        ...
        for node in full_network:
           #need to sed a request to get teh length of all the chains in the network to get the longest one
            ...
            if response.status_code == 200:               #its a standard, operation is succesful, 400 means broken, 500 operation not successufl due to server issue
                                                                                    #5xx range is error in server, 400-500 is user input fault
...
def new_transaction():
    ...
    index = blockchain.new_transaction(json_file["sender"], json_file["receiver"], json_file["amount"]) #need to add transaction ot the next mined block
...
def connect_new_node():
        ...
        #we need to get adresses in the json file before we cqn use the new_node funtion since it requires the new adress as a parameter
        #a sinlge node key ni the json file will have the value as all the node addresses
        all_nodes = json.get("nodes")
        if all_nodes is None: #checking that the listof nodes ins"t empty
...
def replace_chain():
    ...
    if is_chain_replaced:
        ...
        response = {"message": "All good, chain is upto date, the full chain is:",
                               "Full_chain":  list(blockchain.chain)}
    return jsonify(response), 200
...
#cheate a methos called
#check the lenght of transaction, or time limit, combine all transactions and put it all into one blockc